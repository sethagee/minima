...
## Normal linear model
For a normal linear model, we have for a response variable (a $$ m \times 1 $$ vector) $$ y $$, regression coefficients/parameters (a $$ (n+1) \times 1 $$ vector)  $$ \theta $$, and design matrix (a $$ m \times (n+1) $$ matrix) $$ X $$ 
$$ y = X\theta + \varepsilon, $$
